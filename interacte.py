...
class Main(object):
	...
	def __init__(self, params):
		...
		self.load_data()
		self.model        = ConvE(self.p)
		self.model        = self.model.to(self.device)
		self.model2 = InteractE2(self.p, self.chequer_perm)
		self.model2       = self.model2.to(self.device)
    
		self.optimizer    = self.add_optimizer(self.model.parameters())
		self.optimizer2    = torch.optim.Adam(self.model2.parameters(), lr=self.p.lr, weight_decay=self.p.l2)
	...
	def save_model(self, save_path):
		...
		torch.save(state1, save_path+'_model1')
		"""saving model2!!"""
		state2 = {
		...
		torch.save(state2, save_path+'_model2')
	...
	def load_model(self, load_path):
		...
		"""Loading model"""
		state1				= torch.load(load_path+'_model1')
		state_dict1			= state1['state_dict']

		self.model.load_state_dict(state_dict1)
		self.optimizer.load_state_dict(state1['optimizer'])
		"""Loading model2"""
		state2				= torch.load(load_path+'_model2')
		state_dict2			= state2['state_dict']
		self.best_val_mrr 		= state2['best_val']['mrr']
		self.best_val 			= state2['best_val']

		self.model2.load_state_dict(state_dict2)
		self.optimizer2.load_state_dict(state2['optimizer'])

	def evaluate(self, split, lp, epoch=0):
		"""
		...
		"""		
		left_results  = self.predict(split=split, mode='tail_batch', lp=lp)
		right_results = self.predict(split=split, mode='head_batch', lp=lp)
		...
		self.logger.info('Hits@1 : {:.5}, Hits@3 : {:.5}, Hits@10 : {:.5}'.format(results['hits@1'], results['hits@3'], results['hits@10']))
		return results

	def predict(self, split='valid', mode='tail_batch', lp=0):
		"""
		...
		"""
		"""model and model2 are evaluated!"""
		self.model.eval()
		self.model2.eval()
		...
		with torch.no_grad():
			...
			for step, batch in enumerate(train_iter):
				...
				pred1			= self.model.forward(sub, rel, None, lp, 'one_to_n')
				pred2			= self.model2.forward(sub, rel, None, self.model.rm_fea.detach(), 'one_to_n')
				b_range			= torch.arange(pred2.size()[0], device=self.device)
				target_pred		= pred2[b_range, obj]
				pred2 			= torch.where(label.byte(), torch.zeros_like(pred2), pred2)
				pred2[b_range, obj] 	= target_pred
				ranks			= 1 + torch.argsort(torch.argsort(pred2, dim=1, descending=True), dim=1, descending=False)[b_range, obj]
	...
	def run_epoch(self, epoch, lp):
		...
		for step, batch in enumerate(train_iter):
			...
			pred1	= self.model.forward(sub, rel, neg_ent, lp, self.p.train_strategy)
			pred2 = self.model2.forward(sub, rel, neg_ent, self.model.rm_fea.detach(), self.p.train_strategy)
      
			loss1	= self.model.loss(pred1*pred2, label, sub_samp)
			loss2 = self.model2.loss(pred2, label, sub_samp) + self.p.lamb*self.model2.contra_loss(self.p.temperature)
	...
	def fit(self):
		...
		for epoch in range(self.p.max_epochs):
			p = epoch / float(self.p.max_epochs)
			lp = 2./ (1. + np.exp(-10*p)) - 1
      
			train_loss	= self.run_epoch(epoch, lp)
			val_results	= self.evaluate('valid', lp, epoch)
...
if __name__ == "__main__":
	...
	parser.add_argument('--CFR_kernels',  	type=int,      	default=16,    	help='Kernel number of Convolutional Feature Revision')
 
	parser.add_argument('--topK', type=int, default=10, help='The number of postive samples in CL. Default: 10')
	parser.add_argument('--ratio', type=float, default=0.25, help='The ratio of number of negative samples in CL. Default: 0.25')
	parser.add_argument('--temperature', type=float, default=0.05, help='The temperature used in contrastive loss. Default: 0.05.')
	parser.add_argument('--lamb', type=float, default=0.01, help='The lambda used in contrastive loss. Default: 0.01.')
	def str2bool(val):
		return val.lower() in ("yes", "true", "t", "1")
	parser.register('type', 'bool', str2bool)
	parser.add_argument('--descending', type=str2bool, default=False,  help='The order of choosing negative sample: True for descending order.')
